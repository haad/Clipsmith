...
actor SnippetStore {
    ...
    func fetchAll() throws -> [PersistentIdentifier] {
        let descriptor = FetchDescriptor<FlycutSchemaV1.Snippet>(
    ...
    func search(query: String) throws -> [PersistentIdentifier] {
        ...
        let allDescriptor = FetchDescriptor<FlycutSchemaV1.Snippet>(
            sortBy: [SortDescriptor(\.updatedAt, order: .reverse)]
        )
    ...
    func update(
        ...
        guard let snippet = modelContext.model(for: id) as? FlycutSchemaV1.Snippet else {
    ...
    func content(for id: PersistentIdentifier) -> String? {
        return (modelContext.model(for: id) as? FlycutSchemaV1.Snippet)?.content
    }
    ...
    func snippet(for id: PersistentIdentifier) -> SnippetInfo? {
        guard let snippet = modelContext.model(for: id) as? FlycutSchemaV1.Snippet else { return nil }
        return SnippetInfo(
