...
final class ClipboardMonitor {
    ...
    /// that cycle to avoid re-capturing Flycut's own paste write.
    ...
    func checkPasteboard() {
        ...
        // is Flycut itself (e.g. during bezel transition), leave metadata nil to avoid
        // recording "Flycut" as the source of a clipping.
    ...
    ///
    /// Type strings from nspasteboard.org and FlycutOperator.m shouldSkip: logic.
    /// Internal (not private) so tests can call it directly.
    func shouldSkip(pasteboard: NSPasteboard) -> Bool {
        ...
        // filter matching the ObjC Flycut behaviour.
