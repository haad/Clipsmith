...
actor PromptLibraryStore {
    ...
    func update(id: PersistentIdentifier, title: String, content: String) throws {
        guard let item = modelContext.model(for: id) as? FlycutSchemaV2.PromptLibraryItem else {
    ...
    func delete(id: PersistentIdentifier) throws {
        guard let item = modelContext.model(for: id) as? FlycutSchemaV2.PromptLibraryItem else { return }
    ...
    func deduplicate() throws {
        ...
        var seen: [String: FlycutSchemaV2.PromptLibraryItem] = [:]
        for item in all {
    ...
    func content(for id: PersistentIdentifier) -> String? {
        return (modelContext.model(for: id) as? FlycutSchemaV2.PromptLibraryItem)?.content
    }
    ...
    func prompt(for id: PersistentIdentifier) -> PromptInfo? {
        guard let item = modelContext.model(for: id) as? FlycutSchemaV2.PromptLibraryItem else { return nil }
        return PromptInfo(
