...
final class SchemaMigrationTests: XCTestCase {
    ...
    func testV2SchemaIncludesAllFourModels() {
        let modelTypes = FlycutSchemaV2.models
        XCTAssertEqual(modelTypes.count, 4, "FlycutSchemaV2 must include 4 models: Clipping, Snippet, GistRecord, PromptLibraryItem")
    }
    ...
    func testV1ModelsWorkWithV2Schema() async throws {
        ...
        try await MainActor.run {
            ...
            // Insert a Clipping
            let clipping = FlycutSchemaV2.Clipping(content: "test clipping")
            context.insert(clipping)

            // Insert a Snippet
            let snippet = FlycutSchemaV2.Snippet(name: "test snippet", content: "let x = 1")
            context.insert(snippet)

            // Insert a GistRecord
            let gist = FlycutSchemaV2.GistRecord(gistID: "abc123", gistURL: "https://gist.github.com/abc123", filename: "test.swift")
            context.insert(gist)

            try context.save()

            let clippings = try context.fetch(FetchDescriptor<FlycutSchemaV2.Clipping>())
            let snippets = try context.fetch(FetchDescriptor<FlycutSchemaV2.Snippet>())
            let gists = try context.fetch(FetchDescriptor<FlycutSchemaV2.GistRecord>())
